...
impl CommandProcessor {
  pub fn decrypt(src: &PathBuf, dest: Option<&PathBuf>, force: bool) {
    ...
    let dest_path = match dest {
      Some(p) => p,
      None => {
        let ext = match default_dest.extension() {
          Some(n) => n.to_string_lossy().into_owned(),
          None => String::new(),
        };

        if (format!(".{}", &ext)) == DEFAULT_ENCRYPTED_FILE_EXT {
          default_dest.set_extension("");
        }
    ...
    if dest_path.exists() && !force {
      log::warning("destination path exists. Overwrite? [Y/n]");
      let inp_ch = getch();
